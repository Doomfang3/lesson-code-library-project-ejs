...
router.get('/books/:bookId/edit', (req, res, next) => {
  ...
  Book.findById(bookId)
    .then(bookToEdit => {
      ...
      res.render('books/book-edit.ejs', { book: bookToEdit }) // <-- add this line
    })
    .catch(error => next(error))
})
...
router.get('/books', (req, res, next) => {
  Book.find()
    .then(allTheBooksFromDB => {
      ...
      res.render('books/books-list.ejs', { books: allTheBooksFromDB }) // pass `allTheBooksFromDB` to the view (as a variable books to be used in the EJS)
    })
    .catch(error => {
